...
通过编译命令打包生成RegistrationCenter,需要手动在conf添加配置文件`zoo.cfg`
...
initLimit=10
syncLimit=5
dataDir=/home/cirrofs/gaoyuanfeng/data_cache_coverage/RegistrationCenter-3.0.0init-10-g7d5f0f8/.data
clientPort=6348
#maxClientCnxns=60
```
...
| cirrofs_user | CirroFS集群管理员账号，与CirroEngine的cirrofs_user一致 | hdfs 或 hadoop |
| root_name | 与CirroeEngine的root_name保持一致 | xcloud |
| engine_cluster_name | 与CirroeEngine的cluster_name保持一致 | xcloud_3.0 |
...
| cirrofs_user | 访问CirroFS的账号,在同时指定cirrofs_user和principal和keytab的情况下，cirrofs_user会被忽略 | hdfs 或 hadoop |
